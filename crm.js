...
function loadPreviewPdfRenderer(){if(window.html2pdf)return Promise.resolve(window.html2pdf);return new Promise((resolve,reject)=>{const script=window.document.createElement('script');script.src='./assets/html2pdf.bundle.min.js';script.onload=()=>window.html2pdf?resolve(window.html2pdf):reject(new Error('Preview PDF renderer unavailable'));script.onerror=()=>reject(new Error('Preview PDF renderer failed to load'));window.document.head.appendChild(script)})}
async function saveExactPreviewPdf(record){const stage=window.document.createElement('div');stage.style.cssText='position:fixed;left:0;top:0;width:794px;background:#fff;z-index:99999;pointer-events:none';stage.innerHTML=documentHTML(record);window.document.body.appendChild(stage);try{toast('Creating an exact preview PDF…');const html2pdf=await loadPreviewPdfRenderer();await new Promise(resolve=>requestAnimationFrame(()=>requestAnimationFrame(resolve)));await html2pdf().set({margin:0,filename:`${record.number}-preview.pdf`,image:{type:'jpeg',quality:.98},html2canvas:{scale:2,useCORS:true,backgroundColor:'#ffffff',windowWidth:794},jsPDF:{unit:'mm',format:'a4',orientation:'portrait'},pagebreak:{mode:['css','legacy']}}).from(stage.firstElementChild).save();toast(`${record.number}.pdf downloaded.`)}catch(error){toast(error?.message||'Could not create the preview PDF.')}finally{stage.remove()}}
