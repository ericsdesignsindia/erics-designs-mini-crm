...
async function createQuotationPdf(document){
  ...
  // Keep the compact gold wordmark with the emblem so the header contains the full logo.
  text("ERIC'S",margin+1,y+14,5.1,'bold','left',gold);
  text('DESIGNS',margin+1,y+18.5,5.1,'bold','left',gold);
...
async function downloadDocumentPdf(id){const document=documentForDelivery(id);if(!document){toast('Document not found.');return}try{toast('Creating PDF…');const file=await createDocumentPdf(document);downloadPdfFile(file);toast(`${document.number}.pdf downloaded.`)}catch(error){toast(error?.message||'Could not create the PDF. Check your connection and try again.')}}
...
function openDocumentDelivery(id){const record=documentForDelivery(id);if(!record)return;let dialog=window.document.getElementById('documentDeliveryMenu');if(!dialog){dialog=window.document.createElement('dialog');dialog.id='documentDeliveryMenu';dialog.className='client-form';window.document.body.append(dialog)}dialog.innerHTML=`<div class="dialog-title"><div><div class="eyebrow">SEND TO CLIENT</div><h2>${esc(record.number)}</h2><p class="sub">Choose a delivery method. A PDF copy is generated for every option.</p></div><button aria-label="Close" onclick="window.document.getElementById('documentDeliveryMenu').close()">×</button></div><div class="quick-create-grid"><button onclick="window.document.getElementById('documentDeliveryMenu').close();sendDocumentWhatsApp('${record.id}')"><span>◉</span>WhatsApp</button><button onclick="window.document.getElementById('documentDeliveryMenu').close();composeDocumentEmail('${record.id}')"><span>✉</span>Email</button><button onclick="window.document.getElementById('documentDeliveryMenu').close();downloadDocumentPdf('${record.id}')"><span>⇩</span>Download PDF</button></div>`;if(!dialog.open)dialog.showModal()}
...
/* Windows print dialogs do not reliably preview web-app documents. Download the generated PDF instead. */
